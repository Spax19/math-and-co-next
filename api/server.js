...
app.post('/register', async (req, res) => {
    ...
    db.query(sql, [username, email, hashedPassword, userType, verificationToken], (err, result) => {
        if (err) {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) {
                ...
                return res.status(500).send({ message: 'Error sending verification email.', err });
            }
            res.status(200).send({ success: true, message: 'Registration successful! A verification email has been sent.' });
        });
    });
});
...
app.post('/add_event', upload.single('image'), (req, res) => {
    ...
    const { name, time, location, phoneNo, email, date, active, description } = req.body;
    const imagePath = req.file ? req.file.filename : null;

    if (!name || !date || !description || !time || !location || !email || !phoneNo || !imagePath) {
        return res.status(400).send({ error: 'All fields are required' });
    }


    const sql = 'INSERT INTO events (name, description, location, date, phoneNo, email, image, time, active ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
    db.query(sql, [name, description, location, date, phoneNo, email, imagePath, time, active], (err, result) => {
        if (err) {
            return res.status(500).send({ error: 'Database error', details: err });
        }
        res.status(200).send({ message: 'Event added successfully', eventId: result.insertId });
    });
});
...
app.post('/add_distributor', upload.single('image'), (req, res) => {
    ...
    const sql = 'INSERT INTO distributors (name, contacts, address, image) VALUES (?, ?, ?, ?)';
...
app.post('/add_product', upload.single('image'), (req, res) => {
    ...
    const sql = 'INSERT INTO stock (name, boxes, bottles, stockCheck, image) VALUES (?, ?, ?, ?, ?)';
    db.query(sql, [name, boxes, bottles, stockCheck, imagePath], (err, result) => {
        if (err) {
            return res.status(500).send({ error: 'Database error', details: err });
        }
        res.status(200).send({ message: 'Product Stock Added Successfully', eventId: result.insertId });
    });
});
...
app.post('/add_wine', upload.fields([
    ...
    const { name, making, storage, servings, foodPairs, price, origin, description, taste, casePrice } = req.body;

    const wineImagePath = req.files && req.files['image'] ? req.files['image'][0].filename : null;
    const caseImagePath = req.files && req.files['caseImage'] ? req.files['caseImage'][0].filename : null;
    const foodImages = req.files && req.files['foodImages'] ? req.files['foodImages'].map(file => file.filename) : [];
    ...
    if (!name || !making || !storage || !price || !description || foodImages.length === 0 || !origin || !taste || !servings || !foodPairs) {
        return res.status(400).send({ error: 'All fields except images are required.' });
    }

    // SQL query to insert data into the database
    const sql = 'INSERT INTO wines (name, making, storage, price, servings, foodPairs, origin, description, taste, image, foodImages, caseImage, casePrice) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
app.get('/users', (req, res) => {
    const sql = 'SELECT * FROM users WHERE status = "active" AND is_verified = TRUE';
...
app.delete('/delete_user/:userId', (req, res) => {
    ...
    db.query(deleteLogsQuery, [userId], (err, result) => {
        if (err) {
            ...
            return res.status(500).json({ message: "Error deleting logs", details: err });
        }
        ...
        db.query(deleteUserQuery, [userId], (err, result) => {
            if (err) {
                ...
                return res.status(500).json({ message: "Error deleting user", details: err });
            }
...
app.put('/deactivate_user/:userId', (req, res) => {
    ...
    db.query(sql, [userId], (err, result) => {
        if (err) {
            ...
            return res.status(500).json({ message: "Error deactivating user", details: err });
        }
...
app.delete("/delete_wine/:id", (req, res) => {
    ...
    db.query(selectSql, [id], (err, result) => {
        ...
        const imagePath = wine.image ? path.join(__dirname, 'uploads', wine.image) : null;
...
app.put('/edit_product/:id', upload.single('image'), (req, res) => {
    ...
    db.query(sql, values, (err, result) => {
        ...
        if (result.affectedRows > 0) {
            res.status(200).send({ message: 'Product updated successfully', updatedId: productId });
        } else {
...
app.put('/edit_wine/:id', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'foodImages', maxCount: 10 }]), (req, res) => {
    const productId = req.params.id; // Get the product ID from the URL
    const {
        name,
        making,
    ...
    const foodImagePaths = req.files['foodImages'] ? req.files['foodImages'].map(file => file.filename) : [];

    // Check if required fields are provided
    if (!name || !making || !storage || !price || !servings || !foodPairs || !origin || !description || !taste) {
        return res.status(400).send({ error: 'All fields are required' });
    }

    // Prepare the SQL query for updating wine
    let sql = 'UPDATE wines SET name = ?, making = ?, storage = ?, price = ?, servings = ?, foodPairs = ?, origin = ?, description = ?, taste = ?';
    const values = [name, making, storage, price, servings, foodPairs, origin, description, taste];
    ...
    db.query(sql, values, (err, result) => {
        ...
        // Check if the product was updated
        if (result.affectedRows > 0) {
            res.status(200).send({ message: 'Wine updated successfully', updatedId: productId });
        } else {
...
app.put('/edit_distributor/:id', upload.single('image'), (req, res) => {
    ...
    db.query(selectQuery, [id], (err, result) => {
        ...
        db.query(updateQuery, [name, contacts, address, image || distributor.image, id], (err, result) => {
            if (err) {
                console.error('Error updating distributor:', err);
                return res.status(500).json({ message: 'Error updating distributor' });
            }

            return res.status(200).json({ id, name, contacts, address, image: image || distributor.image });
        });
    });
});
...
app.put('/update_event_status/:id', async (req, res) => {
    ...
    try {
        ...
        await db.query('UPDATE events SET active = ? WHERE id = ?', [active, eventId]);
...
app.put('/deactivate_account/:id', async (req, res) => {
    ...
    try {
        // Update the toy (event) in the toy box (database)
        await db.query('UPDATE users SET status = `inactive` WHERE id = ? AND userType = `admin`', [active, eventId]);
        res.status(200).send('Event status updated');
    } catch (error) {
        console.error('Error updating event status:', error);
        res.status(500).send('Error updating event status');
