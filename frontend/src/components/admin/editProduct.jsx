...
const EditProductModal = ({ isOpen, onClose, product, onSave }) => {
  ...
  // Update form data when the 'product' prop changes (when a new product is selected for edit)
  useEffect(() => {
    // Ensure numbers are handled correctly if product prop changes
    if (product) {
      setFormData({
        ...product,
        stock: String(product.stock), // Convert back to string for input value
        price: String(product.price), // Convert back to string for input value
      });
    }
  }, [product]);
  ...
  const handleSubmit = (e) => {
    ...
    if (isNaN(updatedProduct.stock) || isNaN(updatedProduct.price)) {
      toast.error("Stock and Price must be valid numbers.");
      return;
    }
    // --- END FIX ---

    onSave(updatedProduct); // Pass the updated product data
  };
    ...
    >
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-3 flex items-center">
          <Edit className="w-6 h-6 mr-2 text-[#d4b26a]" /> Edit Product
        </h2>
