...
const ProfilePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                setIsLoading(true);
                const res = await axios.get('http://localhost:5174/profile', {
                    headers: {
                ...
                if (res.data.orders) {
                    setOrders(res.data.orders.slice(0, 5)); // Show only 5 most recent orders
                }
            } catch (err) {
    ...
    const handleSaveProfile = async () => {
        try {
            setIsLoading(true);
            await axios.put('http://localhost:5174/profile', userData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    const handlePasswordUpdate = async (e) => {
        ...
        try {
            setIsLoading(true);
            await axios.put('http://localhost:5174/change-password', {
    ...
    const handleAvatarUpload = async (e) => {
        ...
        try {
            ...
            const res = await axios.post('http://localhost:5174/upload-avatar', formData, {
                headers: {
                                    ...
                                    {isEditing && (
                                        <label className="absolute bottom-0 right-0 bg-white p-2 rounded-full shadow-md cursor-pointer hover:bg-gray-100">
                                            <input
                                                ...
                                                accept="image/*"
                                            />
                                            <svg className="w-5 h-5 text-[#d4b26a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="border-b border-gray-200">
                            <nav className="flex -mb-px">
                                <button
                                    ...
                                    className={`py-4 px-6 text-center border-b-2 font-medium text-sm ${activeTab === 'profile' ? 'border-[#d4b26a] text-[#d4b26a]' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                                >
                                    Profile
                                </button>
                                <button
                                    onClick={() => setActiveTab('orders')}
                                    className={`py-4 px-6 text-center border-b-2 font-medium text-sm ${activeTab === 'orders' ? 'border-[#d4b26a] text-[#d4b26a]' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                                >
                                    My Orders
                                </button>
                                <button
                                    onClick={() => setActiveTab('password')}
                                    className={`py-4 px-6 text-center border-b-2 font-medium text-sm ${activeTab === 'password' ? 'border-[#d4b26a] text-[#d4b26a]' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                                >
                                    Change Password
                                </button>
                            </nav>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            {isEditing ? (
                                                ...
                                                <p className="px-3 py-2 bg-gray-50 rounded-md">{userData.username}</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            {isEditing ? (
                                                ...
                                                <p className="px-3 py-2 bg-gray-50 rounded-md">{userData.email}</p>
                                            )}
                                        </div>
                                ...
                                <div>
                                    ...
                                    {orders.length > 0 ? (
                                                    ...
                                                    {orders.map((order) => (
                                                        <tr key={order.id}>
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                                <button className="text-[#d4b26a] hover:text-[#c4a25a] mr-3">View</button>
                                                                <button className="text-[#d4b26a] hover:text-[#c4a25a]">Track</button>
                                                            </td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                        ...
                                        <div className="text-center py-12">
                                            ...
                                            <p className="mt-1 text-sm text-gray-500">Your order history will appear here once you make purchases.</p>
                                            <div className="mt-6">
                                                <button
